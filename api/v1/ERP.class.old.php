	...
	protected function order() {
		...
		if($this->method == 'POST') {
		    ...
		    if($this->verb == "validate") {
    			...
    			$customer = R::getAll("select * from contact where id=?",  array($body->contact_id));
				  foreach($customer as $cus) {
					  $contact_email=$cus["email"];
					  $email_isdeclined=$cus["isdeclined"];
					  $customer_name= $cus["firstname"].' '.$cus["lastname"];
				  }    			
    			if(isset($body->declineditems)){
    				if($email_isdeclined==false){
    			    $mail = new PHPMailer();
              $mail->isSMTP();
              ...
              $mail->Password = MAILPASSWORD;
    			  ...
    			  foreach($body->declineditems as $i) {
    			    $item = R::dispense('declinedorderitem');
    			    $item->order_id = $id;
    			    $item->itemtype_id = $i->type;
    			    $item->partnumber = $i->partnumber;
   		        $item->description = $i->description;
    			    ...
    			    $item->cost = $i->cost;
              R::store($item);
    				  $this->replicateData("POST", "declinedorderitem", $item->__toString(), 0);
              if(!$email_isdeclined){
                $mail->setFrom('pruebasrobetorr@gmail.com', 'Flux Shop');
                $mail->addAddress($contact_email, $customer_name);
                $mail->Subject = 'Flux Shop Reminder to '.$customer_name ;
                $mail->Body = "You have declined ".$i->description." service from order number: ".$id;
                $mail->send();
              }
    			  }
    		  }
			...
			if($this->verb == "open") {
				...
				foreach($order as $o) {
					...
					$obj->items = $itemList;
					$declinedorderitem  = R::find('declinedorderitem', ' order_id = ? ', array($this->args[0]) );
					$declineditemList = array();
					foreach($declinedorderitem as $d) {
						$declineditemList[] = $d;
					}
					$obj->declineditems = $declineditemList;					
			...
			if($this->verb == "orderDelete") {
    			...
    			if(isset($body->declineditems)){
    			$declinedorderitems  = R::find('declinedorderitem', ' order_id = ? ', array( $id ) );
				foreach($declinedorderitems as $di) {
				  R::trash($di);
                }
    			foreach($body->declineditems as $i) {
    				$item = R::dispense('declinedorderitem');
	...
	protected function inventory() {
		...
		if($this->method == 'POST') {
					...
					if (($handle = fopen($_FILES['inventory_csv_file']['tmp_name'], 'r')) !== FALSE) {
						while (($row = fgetcsv($handle)) !== FALSE) {
							if (in_array($row[1], $partnumbers)) {
								...
								$inventory->cost = $row[3];
								$inventory->retail = $row[4];
			...
			if($this->verb == "all") {
				...
				return $returnArray;
			} elseif($this->verb == "historytrail") {
				$returnArray = array();
				$inventoryid=$_GET['id'];
				$returnArray['items'] = R::getAll("SELECT i.id, i.manufacturer, i.partnumber, i.description,oi.order_id, CONCAT(c.firstname, ', ',c.lastname) as customername, DATE(o.updated) as date,i.cost as inventorycost,oi.cost as ordercost, oi.quantity, v.vendorname FROM `inventory` i LEFT JOIN orderitem oi ON i.partnumber = oi.partnumber LEFT JOIN `order` o ON oi.order_id = o.id LEFT JOIN `contact` c ON o.contact_id = c.id LEFT JOIN `vendor` v ON oi.vendor_id = v.id WHERE i.id = ?", array($inventoryid));
				return $returnArray;
	...
	protected function user() {
		if($this->method == 'POST') {
			...
			if($this->verb == "authenticate") {
				...
				$usr = R::findOne('user', ' email = ? ', array($body['username']) );
	...
	protected function report() {
		...
		if($this->method == 'GET'){
			if($this->verb == "postedtotals") {
				...
				return $retArray;
			} elseif ($this->verb == "reservedInventory") {
				if(!isset($_GET['start'])) {
					return "Missing parameter: start";
				}
				if(!isset($_GET['end'])) {
				...
				$rows = R::getAll("SELECT o.id, i.manufacturer, i.partnumber, i.description, CONCAT(c.firstname, ', ',c.lastname) as customername, o.customernotes, DATE(o.updated) as date, oi.quantity FROM `inventory` i LEFT JOIN orderitem oi ON i.partnumber = oi.partnumber LEFT JOIN `order` o ON oi.order_id = o.id LEFT JOIN `contact` c ON o.contact_id = c.id WHERE i.reserved > 0 and oi.itemtype_id in (3,4) and o.updated >= ? and o.updated <= ?", array($start, $end));
				$items_reserved = R::convertToBeans( 'order', $rows );
				...
				$columns[] = array("data"=>"partnumber", "defaultContent"=>"", "label"=>"Part Number");
				$columns[] = array("data"=>"description", "defaultContent"=>"", "label"=>"Description");
				$columns[] = array("data"=>"id", "defaultContent"=>0, "label"=>"Order Number");
				$columns[] = array("data"=>"customer", "defaultContent"=>"", "label"=>"Customer Name");
				$columns[] = array("data"=>"customernotes", "defaultContent"=>0, "label"=>"Customer Notes");
				$columns[] = array("data"=>"date", "defaultContent"=>"", "label"=>"Date");
				$columns[] = array("data"=>"quantity", "defaultContent"=>0, "label"=>"Quantity");

				if($items_reserved == null) {
					return array("columns"=>$columns, "data" => $retArray);
				}

				foreach($items_reserved as $i) {
					$retArray[] = $i;
				}
				if(isset($_GET['format']) && $_GET['format'] == "datatable") {
					return array("columns"=>$columns, "data" => $retArray);
