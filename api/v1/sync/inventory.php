...
if(TWO_WAY_SYNC) {
	$tables = ['inventory', 'vendor', 'invoice', 'invoiceitem'];
	$i = 1;
	foreach ($tables as $table) {
		try {
			...
			foreach ($response as $res) {
				...
				if ($res['table'] == 'invoice') {
					$vendor = R::findOne('vendor', ' id = ? or central_id = ?', array($res['data']['vendor_id'], $res['data']['vendor_id']));
					if (!$vendor) {
				...
				if ($res['table'] == 'invoiceitem') {
					$invoice = R::findOne('invoice', ' id = ? or central_id = ?', array($res['data']['invoice_id'], $res['data']['invoice_id']));
					$inventory = R::findOne('inventory', ' id = ? or central_id = ?', array($res['data']['inventory_id'], $res['data']['inventory_id']));
					if (!$invoice || !$inventory) {
						echo "Unable to process: (". $res['table'] . ") with Central ID: (". $res['id'] ."), Invoice with ID: (". $res['data']['invoice_id'] .") or Inventory with ID: (". $res['data']['inventory_id'] .") does not exists on local\n";
						continue;
					}

					// Setting foreignKeys to local ids
					$res['data']['invoice_id'] = $invoice->id;
					$res['data']['inventory_id'] = $inventory->id;
				}
	...
	$updt_tables = ['inventory', 'vendor', 'invoice'];
